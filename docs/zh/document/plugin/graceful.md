...
**延迟下线：** 即对下线的实例提供保护，插件基于**下线实时通知**+**刷新缓存的机制**快速更新上游的实例缓存，同时基于流量统计的方式，确保即将下线的实例尽可能的将流量处理完成，最大程度避免流量丢失。
...
其中consumer开启优雅上下线能力，一个provider实例开启预热与优雅下线能力， 另一个provider实例仅开启优雅下线能力，data服务接受到请求后会返回自身服务名+IP地址信息。
