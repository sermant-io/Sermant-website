...
| 治理规则 | 前缀 |
| -- | -- |
| 流量标记 | servicecomb.MatchGroup |
| 限流 | servicecomb.rateLimiting |
| 熔断 | servicecomb.circuitBreaker |
| 隔离 | servicecomb.bulkhead |
| 重试 | servicecomb.retry |
| 错误注入 | servicecomb.faultInjection |
...
以zookeeper为例，利用PrettyZoo工具来发布流量标记策略和流控策略：

1. 创建节点`/service=spring-flow-provider`

<MyImage src="/docs-img/flowcontrol-create-node-1.jpg"/>

2. 创建节点`/service=spring-flow-provider/servicecomb.matchGroup.sceneFlow`和数据`"alias: scene\nmatches:\n- apiPath:\n    exact: /flow\n  headers: {}\n  method:\n  - GET\n  name: flow\n"`

<MyImage src="/docs-img/flowcontrol-create-node-2.jpg"/>

3. 创建节点`/service=spring-flow-provider/servicecomb.rateLimiting.sceneFlow`和数据`"limitRefreshPeriod: \"2S\"\nname: flow\nrate: \"4\"\n"`
